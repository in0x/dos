/*
  ...
  Fixes:
   ...
   NOTE(Phil): I've super broken the C compatiblity of my version, use at your own risk.
*/
...
{
	return std::sqrt((lhv.X - rhv.X) * (lhv.X - rhv.X) + (lhv.Y - rhv.Y) * (lhv.Y - rhv.Y) + (lhv.Z - rhv.Z) * (lhv.Z - rhv.Z) + (lhv.W - rhv.W) * (lhv.W - rhv.W));
}
...
HINLINE void HMM_Transform(const hmm_sphere& inSphere, hmm_sphere& outSphere, const hmm_mat4& matrix)
{
	...
	outSphere.radius = HMM_Distance(outSphere.center, edge);
}
